...
/// Класс для работы с SharedPreferences и обновления токенов.
...
class SharedPrefManager {
  ...
  Future<void> saveDarkThemeEnabled(bool enabled) async {
    await _prefs?.setBool(_darkThemeKey, enabled);
  }

  /// Возвращает состояние тёмной темы.
  bool getDarkThemeEnabled() {
    return _prefs?.getBool(_darkThemeKey) ?? false;
  }
  ...
  /// Проверяет, обновились ли новости.
  ...
  /// Проверяет, обновились ли события.
  ...
  /// Проверяет, обновилось ли расписание студента.
  /// Параметр [date] можно использовать для запроса данных на определённую дату.
  ...
  Future<void> clearTokens() async {
    ...
    await _prefs?.remove(_darkThemeKey);
  }
}
