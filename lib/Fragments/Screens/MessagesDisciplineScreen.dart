...
class MessagesDisciplineScreen extends StatefulWidget {
  ...
  @override
  _MessagesDisciplineScreenState createState() =>
      _MessagesDisciplineScreenState();
}
...
class _MessagesDisciplineScreenState extends State<MessagesDisciplineScreen> {
  ...
  void _loadMessages() {
    setState(() {
      _futureMessages =
          dataApi.getForumMessage(_authHeader, widget.idDiscipline);
    });
  }
  ...
  Future<void> _sendMessage() async {
    ...
    try {
      await dataApi.postForumMessage(
          _authHeader, widget.idDiscipline, message);
  ...
  Future<void> _deleteMessage(int messageId) async {
    try {
      ...
      if (_notificationsEnabled) {
        await _showDeletedNotification(messageId);
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _showDeletedNotification(int messageId) async {
    const AndroidNotificationDetails androidDetails = AndroidNotificationDetails(
      'delete_channel_id',
      'Delete Notifications',
      channelDescription: 'Уведомления об удалении сообщений',
    ...
    await _flutterLocalNotificationsPlugin.show(
      messageId,
      'Сообщение удалено',
      'Ваше сообщение было успешно удалено',
      platformDetails,
    );
  }


  Future<void> _confirmDeleteMessage(int messageId, String text) async {
    final shouldDelete = await showDialog<bool>(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content:
          const Text("Сообщение будет удалено навсегда. Вы уверены?"),
          actions: [
            TextButton(
  ...
  Widget _buildMessageCard(ForumMessage message) {
    ...
    bool isMyMessage =
        _currentUser != null && message.user.id == _currentUser!.id;
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.only(
            ...
            bottomRight:
            isMyMessage ? Radius.zero : const Radius.circular(12),
          ),
        ),
        child: Column(
              ...
              children: [
                ...
                if (isMyMessage)
                  IconButton(
                    ...
                    onPressed: () =>
                        _confirmDeleteMessage(message.id, message.text),
                  ),
              ],
            ),
          ],
  ...
  List<Widget> _buildGroupedMessages(List<ForumMessage> messages) {
    ...
    for (var message in messages) {
      ...
      if (lastDate == null ||
          messageDateOnly.difference(lastDate).inDays != 0) {
        widgets.add(
          Padding(
  ...
  void _scrollToBottom() {
    if (_scrollController.hasClients) {
      // Добавляем небольшой отступ, чтобы показать последнее сообщение
          ...
          Expanded(
            child: FutureBuilder<List<ForumMessage>>(
              ...
              builder: (context, snapshot) {
                ...
                _buildGroupedMessages(snapshot.data!);
