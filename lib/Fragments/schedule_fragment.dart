...
import 'package:mrsu/ResourceServer/TimeTable/TimeTableLesson.dart';
import 'package:table_calendar/table_calendar.dart';

import '../TimeTable/StudentTimeTable.dart';
import '../Token/SharedPrefManager.dart';
import 'Screens/DisciplineScreen.dart'; // Предполагается, что этот экран существует

/// Пример виджета расписания, аналогичного ScheduleScreen из Android
...
class _ScheduleScreenState extends State<ScheduleScreen> {
  ...
  // Цвета: для заголовка факультета – чуть светлее, для карточек занятий – как требовалось
  ...
  /// Создание строки для отображения дисциплины с обработкой нажатия.
  /// Используется как часть объединённого Card, когда несколько дисциплин в одной паре.
  ...
  Widget _buildTimeTable() {
    ...
    for (var studentTT in _studentTimeTable) {
      ...
      int lessonIndex = 0;
      // Для каждого временного интервала (пары)
      for (int i = 0; i < times.length; i++) {
        if (lessonIndex < lessons.length &&
          ...
          // Если для данного интервала нет дисциплин, создаём карточку с сообщением «Нет данных»
          scheduleWidgets.add(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(
        children: [
          // Календарь с русской локалью и переключением формата, где кнопки показывают текущий режим
          TableCalendar(
            locale: 'ru_RU',
