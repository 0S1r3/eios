...
class _NewsScreenState extends State<NewsScreen> {
  ...
  void initState() {
    ...
    // Загружаем общий список событий и сохраняем их
    ...
    // Загружаем события для выбранной даты один раз и сохраняем в Future
    _futureEventsForToday = loadEventsForDay(_selectedDay!);

    // Проверяем наличие обновлений для новостей и событий
    _checkForUpdates(_authHeader);
  }

  // Метод для фильтрации событий по выбранной дате
  List<Events> filterEventsByDate(List<Events> events, DateTime day) {
  ...
  // Метод для загрузки событий для конкретного дня
  Future<List<Events>> loadEventsForDay(DateTime day) async {
    final formattedDate = DateFormat('yyyy-MM-dd').format(day);
    try {
      // Попытка загрузить события через API
      ...
      // Если не удалось, получаем кэшированные данные и фильтруем их
      final cachedEvents = _manager.getEvents() ?? [];
      final filteredEvents = filterEventsByDate(cachedEvents, day);
      setState(() {
        _eventsMap[DateTime(day.year, day.month, day.day)] = filteredEvents;
      });
  ...
  Future<void> _selectDate(BuildContext context) async {
    ...
    if (pickedDate != null && pickedDate != _selectedDay) {
      setState(() {
        ...
        // Обновляем Future только при выборе новой даты
        _futureEventsForToday = loadEventsForDay(pickedDate);
      });
    }
  }
  ...
  void _refreshEvents() async {
    if (_selectedDay != null) {
      final events = await loadEventsForDay(_selectedDay!);
                ...
                Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 8),
                      // FutureBuilder для событий на выбранную дату, использующий сохранённый Future
                      Container(
                        height: 150,
